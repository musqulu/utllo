...
import { LetterText, Copy, Check, Trash2, Clock, BarChart3 } from "lucide-react";
...
import { getWordStats, getTopWords, formatTime } from "@/lib/text-counter";

interface WordCounterDictionary {
  title: string;
  subtitle: string;
  placeholder: string;
  words: string;
...
interface WordCounterProps {
  dictionary: WordCounterDictionary;
}

export function WordCounter({ dictionary }: WordCounterProps) {
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="p-4 rounded-lg bg-muted/50">
            <p className="text-xs text-muted-foreground">{dictionary.avgWordLength}</p>
            <p className="text-xl font-bold">{stats.avgWordLength}</p>
          </div>
          <div className="p-4 rounded-lg bg-muted/50">
            <p className="text-xs text-muted-foreground">{dictionary.avgSentenceLength}</p>
            <p className="text-xl font-bold">{stats.avgSentenceLength} {dictionary.wordsLabel}</p>
          </div>
        </div>
        ...
        {topWords.length > 0 && (
          <div className="p-4 rounded-lg bg-muted/50">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-sm font-medium">{dictionary.topWords}</p>
            </div>
            <div className="flex flex-wrap gap-2">
              {topWords.map(({ word, count }) => (
                <span
                  key={word}
                  className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-background text-sm"
                >
                  <span>{word}</span>
